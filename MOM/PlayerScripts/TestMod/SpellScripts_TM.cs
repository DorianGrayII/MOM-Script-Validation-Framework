    ...
    public class SpellScripts : ScriptBase
    {
        ...
        static public bool STAR_BlackChannels(SpellCastData data, object target, Spell spell)
        {
            if (target is MOM.Unit)
            {
                ...
                if (EnchAlreadyOnObject(data.caster, spell, u)) return false;
                if (u.GetSkills().Contains((Skill)SKILL.BLACK_CHANNELS)) return false;
        ...
        static public bool STAR_ChangeTerrain(SpellCastData data, object target, Spell spell)
        {
            ...
            if (plane != null)
            {
                var hexes = plane.GetHexes();
                if (hexes != null)
                {
                    Debug.Log("Arcanus Hex Count = " + hexes.Count);
                    var area = plane.area;
                    Debug.Log("Arcanus Size = " + plane.area.ToString());
        ...
        static public bool SBG_FireBall(SpellCastData data, object target, Spell spell)
        {
            ...
            var unit = target as BattleUnit;
            dmg = ProduceAreaSpellDamage(dmg, unit.figureCount);
...
//             }
        ...
        static public bool SWG_FireStorm(ISpellCaster source, object target, Spell spell)
        {
            ...
            {
                dmg = ProduceAreaSpellDamage(dmg, units[i].Get().figureCount);
        ...
        static public bool SWG_SpellOfReturn(ISpellCaster source, object target, Spell spell)
        {
            ...
            caster.SetWizardTowerBonus(town);
 ...
 //           spellCaster.SetWizardTowerBonus(town);
        ...
        static public bool SBW_FlameStrike(SpellCastData data, object target, Spell spell)
        {
            ...
            foreach (var u in data.GetEnemyUnits())
            {
                ...
                dmg[0] = spell.fIntData[0].ToInt();
                dmg = ProduceAreaSpellDamage(dmg, u.figureCount);
...
//                 }
		...
		static public void MarkVisible(MOM.FOW fow, Color[] data, int id, Vector3i position)
		{
			if (data[id].r < 1f)
			{
				if (fow.GetArcanusData() == data)
				{
					DiscoverPosition(fow, World.GetArcanus(), position);
				}
				else
				{
					DiscoverPosition(fow, World.GetMyrror(), position);
				}
			}
			data[id] = Color.white;
		}

        static private void DiscoverPosition(MOM.FOW fow, global::WorldCode.Plane plane, global::MHUtils.Vector3i pos)
        {
            global::MOM.Location location = global::MOM.GameManager.GetLocationsOfThePlane(plane).Find((global::MOM.Location o) => !o.discovered && o.GetPosition() == pos);
            if (location != null)
            {
                location.MakeDiscovered();
            }
            global::MOM.Group group = global::MOM.GameManager.GetGroupsOfPlane(plane).Find((global::MOM.Group o) => o.GetPosition() == pos);
            if (group != null && group.locationHost == null)
            {
                group.GetMapFormation(true);
            }
            global::WorldCode.Hex hexAt = plane.GetHexAt(pos);
            if (((hexAt != null) ? hexAt.Resource : null) != null)
            {
                global::MHUtils.UI.VerticalMarkerManager.Get().Addmarker(hexAt);
            }
        }
        static public void MarkVisible(MOM.FOW fow, Vector3i pos, bool arcanus)
		{
			int width = fow.GetDatatexture().width;
			int height = fow.GetDatatexture().height;
			Color[] data = arcanus ? fow.GetArcanusData() : fow.GetMyrrorData();
            int num = ((int)pos.x + width) % width;
            if (num < 0)
               num += width;
            int num2 = ((int)pos.y + height) % height;
            if (num2 < 0)
               num2 += height;
            int offset = num + num2 * width;
            Debug.Log("calling MarkVisible datalength = " + data.GetLength(0) + " offset = " + offset + " pos = " + pos.ToString() +
                       " width = " + width + " height = " + height + " num = " + num + " num2 = " + num2);
			MarkVisible(fow, data, offset, pos);
		}
        ...
        static public bool SWO_EarthLore(ISpellCaster source, Spell spell, object target, object data)
        {
            if (target is Vector3i)
            {
                ...
                var area = p.GetPositionWrapping(HexNeighbors.GetRange(v3i, 50));
                ...
                if (fow != null)
                {
                    ...
                    {
                        MarkVisible(fow, v, p.arcanusType);
        ...
        static public int SWAI_WaterWalk(ISpellCaster source, object target, Spell spell)
        {
            ...
            if (destination.inNeedOfWaterTransport)
            {
                //Average spell value based on target
                value = buValue / 3;
            }
        ...
        static public int SWAI_WindWalking(ISpellCaster source, object target, Spell spell)
        {
                ...
                group.aiDesignation.inNeedOfWaterTransport)
            {
                if (group.GetUnits().Count < 3)
                    value = 500 + (int)(unit.GetWorldUnitValue() / 2);
                else if (group.GetUnits().Count < 6)
                    value = 1000 + (int)(unit.GetWorldUnitValue() / 2);
                else
                    value = 1500 + (int)(unit.GetWorldUnitValue() / 2);
            }
...
//            int value = 0;
        ...
        static public int SWAI_Disjunction(ISpellCaster source, object target, Spell spell)
        {
            ...
            {
                //enemy effect on self
                return 500 + ei.upkeepMana * 50;
            }
            else
            {
                return 300 + ei.upkeepMana * 50;
            }
        }
        ...
        public static int[] ProduceAreaSpellDamage(int[] dmg, int figures)
        {
            var baseDmg = dmg[0];
